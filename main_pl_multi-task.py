...
test_centers_list = ['bwh', 'karo', 'milan', 'rennes', 'nih', 'montpellier', 'ucsf', 'amu']
centers_list = ['mix']
...
class Model(pl.LightningModule):
    def __init__(self, args, center_name, data_root, optimizer_class, load_pretrained=False, 
    ...
    def prepare_data(self):
        ...
        self.test_datasets_list = get_test_datasets(test_centers_list, self.root, test_transforms, self.args.seed)
    ...
    def train_dataloader(self):
        ...
        return DataLoader(self.train_ds, batch_size=args.batch_size, shuffle=True, num_workers=4, 
    ...
    def test_step(self, batch, batch_idx, dataloader_idx):
        ...
        return {test_centers_list[dataloader_idx]: [test_soft_dice, test_hard_dice, test_precision, test_recall]}
    ...
    def test_epoch_end(self, outputs):
        ...
        for i in range(len(outputs)):
            avg_soft_dice_test[test_centers_list[i]] = (torch.stack([ x[test_centers_list[i]][0] for x in outputs[i] ]).mean()).cpu().numpy()
            avg_hard_dice_test[test_centers_list[i]] = (torch.stack([ x[test_centers_list[i]][1] for x in outputs[i] ]).mean()).cpu().numpy()
            
            avg_precision_test[test_centers_list[i]] = (np.stack([ x[test_centers_list[i]][2] for x in outputs[i] ]).mean())
            avg_recall_test[test_centers_list[i]] = (np.stack([ x[test_centers_list[i]][3] for x in outputs[i] ]).mean())

        logger.info(f"Test (Soft) Dice for centers {test_centers_list}: {avg_soft_dice_test}")
        logger.info(f"Test (Hard) Dice for centers {test_centers_list}: {avg_hard_dice_test}")
        logger.info(f"Test Precision Score for centers {test_centers_list}: {avg_precision_test}")
        logger.info(f"Test Recall Score for centers {test_centers_list}: {avg_recall_test}")
...
def main(args):
    ...
    save_path = os.path.join(save_path, centers_order, f"MT_seed={args.seed}")
    ...
    for i, center in enumerate(centers_list):
        ...
        save_exp_id = f"{center}_MT_se={args.seed}_{timestamp[4:]}"
        ...
        if pretrained_load_path is not None:
            pl_model = Model(args, center_name=center, data_root=dataset_root, optimizer_class=optimizer_class, 
                                load_pretrained=True, load_path=pretrained_load_path, exp_id=save_exp_id, replay=True)
        else:
            # i.e. train on the first center by loading weights from scratch
            pl_model = Model(args, center_name=center, data_root=dataset_root, optimizer_class=optimizer_class, 
                                load_pretrained=False, load_path=pretrained_load_path, exp_id=save_exp_id, replay=False)
