...
from monai.networks.nets import UNet, AttentionUnet, BasicUNet, SegResNet
...
class Model(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx, dataloader_idx):
        ...
        # upon fsleyes visualization, observed that very small values need to be set to zero, but NOT fully binarizing d
        ...
        #                 self.args.results_dir, centers_order, f"seed={self.args.seed}", self.save_exp_id
        #                 )
...
def main(args):
    ...
    save_path = os.path.join(save_path, centers_order, f"FT_seed={args.seed}")
    ...
    for i, center in enumerate(centers_list):
        ...
        save_exp_id = f"{center}_se={args.seed}_{timestamp[4:]}"
        ...
        if pretrained_load_path is not None:
            pl_model = Model(args, centers_list=centers_list, center_name=center, data_root=dataset_root, optimizer_class=optimizer_class, 
                                load_pretrained=True, load_path=pretrained_load_path, exp_id=save_exp_id)
        else:
            # i.e. train on the first center by loading weights from scratch
            pl_model = Model(args, centers_list=centers_list, center_name=center, data_root=dataset_root, optimizer_class=optimizer_class, 
                                load_pretrained=False, load_path=pretrained_load_path, exp_id=save_exp_id)
        ...
        # NOTE: this load_from_checkpoint method is making it execute the previous "print" statements that appear in-between
        # in the terminal
