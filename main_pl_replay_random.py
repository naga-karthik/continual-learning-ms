...
class Model(pl.LightningModule):
    def __init__(self, args, centers_list, center_name, data_root, optimizer_class, load_pretrained=False, 
    ...
    def prepare_data(self):
        ...
        if self.center_idx == 0:
            ...
            for i in range(1, self.center_idx + 1):
                ...
                prev_dataset =  self.root + f"dataset_{prev_center_name}.json"
                ...
                if len(prev_train_files) < args.num_replay_samples_per_center:
                    self.buffer += prev_train_files
                else:
                    self.buffer += prev_train_files[:args.num_replay_samples_per_center]
        ...
        self.test_datasets_list = get_test_datasets(self.centers_list, self.root, test_transforms, self.args.seed)
    ...
    def test_epoch_end(self, outputs):
        ...
        # with open(os.path.join(self.predictions_save_path, f"test_dice_{self.center_name}.txt"), 'a') as f:
        #     print(f"\n------------ Training on center {self.center_name}; Test Dice Scores from all Centers ------------", file=f)
        #     print(f"{avg_soft_dice_test}", file=f)
        #     print('----------------------------------------------------------------------', file=f)
...
def main(args):
    ...
    save_path = os.path.join(save_path, centers_order, f"ER_seed={args.seed}")
    ...
    for i, center in enumerate(centers_list):
        ...
        save_exp_id = f"{center}_ER_se={args.seed}_{timestamp[4:]}"

        if pretrained_load_path is not None:
            pl_model = Model(args, centers_list=centers_list, center_name=center, data_root=dataset_root, optimizer_class=optimizer_class, 
        ...
        # NOTE: this load_from_checkpoint method is making it execute the previous "print" statements that 
        # appear in-between in the terminal
